class StuffDataFromGoogleSpreadsheet < ActiveRecord::Migration
  def up
    if_and_only_if_the_database_is_empty
  end

  def down
  end
  
  def if_and_only_if_the_database_is_empty
    if (0 == Story.all.length) then
  ...
  def stuff_google_spreadsheet_data_into_database
    ...
    tales.each {|ns| Story.new(as_a: ns[0], i_want_to: ns[1], so_that: ns[2], notes: ns[3]).save}
  end
    
  def set_all_initial_status_to_backlog
    Story.all.each {|s| s.status = "Backlog"; s.save}
  end
end
